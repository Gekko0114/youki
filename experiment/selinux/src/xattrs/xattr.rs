...
pub fn set_xattr(fpath: &Path, attr: &str, data: &[u8]) -> Result<(), XattrError> {
    match rfs::setxattr(fpath, attr, data, rfs::XattrFlags::CREATE) {
        Ok(_) => Ok(()),
        Err(e) => {
...
pub fn lset_xattr(fpath: &Path, attr: &str, data: &[u8]) -> Result<(), XattrError> {
    match rfs::lsetxattr(fpath, attr, data, rfs::XattrFlags::CREATE) {
        Ok(_) => Ok(()),
        Err(e) => {
            let errno = e.raw_os_error();
            if errno == libc::EINTR {
...
pub fn get_xattr(fpath: &Path, attr: &str) -> Result<String, XattrError> {
    ...
    loop {
        match rfs::getxattr(fpath, attr, &mut buf) {
            Ok(size) => {
                if size == buf_size {
                    buf_size *= 2;
...
pub fn lget_xattr(fpath: &Path, attr: &str) -> Result<String, XattrError> {
    ...
    loop {
        match rfs::lgetxattr(fpath, attr, &mut buf) {
            Ok(size) => {
                if size == buf_size {
                    buf_size *= 2;
                    buf.resize(buf_size, 0);
...
mod tests {
    ...
    fn test_set_xattr_and_get_xattr() {
        ...
        set_xattr(&file_path, attr_name, attr_value.as_bytes()).expect("Failed to set xattr");
        let actual = get_xattr(&file_path, attr_name).expect("Failed to get xattr");
